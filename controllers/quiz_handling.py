...
@login_required
def view_quiz(id):
    # Fetch the quiz by ID from the database
    quiz = Quiz.query.get(id)
    if not quiz:
        flash('Quiz not found!', 'error')
        return redirect(url_for('user_dashboard'))  # Redirect if quiz not found

    # Get chapter and subject details for display
    chapter = quiz.chapter
    subject = chapter.subject if chapter else None
    question_count = Question.query.filter_by(quiz_id=quiz.id).count()  # Count total questions

    # Prepare quiz details for rendering
    quiz_details = {
        "id": quiz.id,
...
def start_quiz(quiz_id, question_index):
    ...
    if request.method == 'POST':
        ...
        if user_answer:
            session[f'quiz_{quiz_id}_q{question_index}'] = user_answer  # Store answer in session
...
def submit_quiz(quiz_id):
    ...
    # Clear session data and store feedback
...
@login_required
def scores():
    # Retrieve quiz ID from request arguments
    quiz_id = request.args.get('quiz_id')

    # Get feedback and score data from session
    ...
    # Fetch all quiz attempts by current user
    quiz_attempts = Score.query.filter_by(user_id=current_user.id).all()

    # Prepare summary data for all attempts
    quiz_summary_data = []
    ...
    return render_template('scores.html', attempts=quiz_summary_data, total_attempted_quizzes=total_attempted_quizzes,
                           ...
                           total_questions=total_questions, feedback_list=feedback_list)
